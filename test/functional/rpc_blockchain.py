...
class BlockchainTest(BitcoinTestFramework):
    ...
    def _test_getchaintxstats(self):
        ...
        assert_raises_rpc_error(-1, "Expected type number, got string", self.nodes[0].getchaintxstats, '')
        assert_raises_rpc_error(-8, "Invalid block count: should be between 0 and the block's height - 1", self.nodes[0].getchaintxstats, -1)
        assert_raises_rpc_error(-8, "Invalid block count: should be between 0 and the block's height - 1", self.nodes[0].getchaintxstats, self.nodes[0].getblockcount())

        # Test `getchaintxstats` invalid `blockhash`
        assert_raises_rpc_error(-1, "Expected type string, got number", self.nodes[0].getchaintxstats, blockhash=0)
    ...
    def _test_getblock(self):
        ...
        assert_raises_rpc_error(-1, "Expected type number, got string", node.getblock, blockhash, "2")
