...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def connect_nodes(self, a, b, *, peer_advertises_v2=None, wait_for_connect: bool = True):
        ...
        def find_conn(node, peer_subversion):
            return next(filter(lambda peer: peer['subver'] == peer_subversion, node.getpeerinfo()), None)
        ...
        self.wait_until(lambda: find_conn(from_connection, to_connection_subver) is not None)
        self.wait_until(lambda: find_conn(to_connection, from_connection_subver) is not None)

        self.wait_until(lambda: (peer := find_conn(from_connection, to_connection_subver)) is not None and peer['bytesrecv_per_msg'].pop('verack', 0) >= 21)
        self.wait_until(lambda: (peer := find_conn(to_connection, from_connection_subver)) is not None and peer['bytesrecv_per_msg'].pop('verack', 0) >= 21)

        # The message bytes are counted before processing the message, so make
        # sure it was fully processed by waiting for a ping.
        self.wait_until(lambda: (peer := find_conn(from_connection, to_connection_subver)) is not None and peer["bytesrecv_per_msg"].pop("pong", 0) >= 29)
        self.wait_until(lambda: (peer := find_conn(to_connection, from_connection_subver)) is not None and peer["bytesrecv_per_msg"].pop("pong", 0) >= 29)
