...
class TxConflicts(BitcoinTestFramework):
    ...
    def get_utxo_of_value(self, from_tx_id, search_value):
        return next(tx_out["vout"] for tx_out in self.nodes[0].gettransaction(from_tx_id)["details"] if tx_out["amount"] == Decimal(search_value))
    ...
    def run_test(self):
        ...
        self.log.info("Create transactions that conflicts between each other")
        ...
        output_c = self.get_utxo_of_value(txid_AB_parent, 19.99998)
        ...
        self.sync_blocks()
        conflicted = self.nodes[0].gettransaction(conflicted_AB_tx)
        tx_C_child = self.nodes[0].gettransaction(tx_C_child_txid)
        ...
        self.sync_blocks()
        former_conflicted = self.nodes[0].gettransaction(conflicted_AB_tx)
