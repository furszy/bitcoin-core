...
public:
     ...
     */
    boost::signals2::signal<void (uint256 spkm_id, size_t range_start, size_t range_end)> NotifyNewScriptsAdded;
};
...
public:
    ...
    std::unordered_set<CScript, SaltedSipHasher> GetScriptPubKeys() const override;
    std::unordered_set<CScript, SaltedSipHasher> GetScriptPubKeys(int32_t minimum_index, int32_t max_index = std::numeric_limits<int32_t>::max()) const;
