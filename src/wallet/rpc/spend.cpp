...
void FundTransaction(CWallet& wallet, CMutableTransaction& tx, CAmount& fee_out, int& change_position, const UniValue& options, CCoinControl& coinControl, bool override_min_fee)
{
    ...
    if (!options.isNull()) {
      if (options.type() == UniValue::VBOOL) {
        ...
        // By default, add other inputs
        coinControl.fAllowOtherInputs = options.exists("add_inputs") ? options["add_inputs"].get_bool() : true;
        // For now, 'm_use_manual_selection' is always equal to 'add_inputs'.
        // Could be customized in the future so the wallet can select the best matching UTXOs from a user provided UTXO set.
        coinControl.m_use_manual_selection = coinControl.fAllowOtherInputs;
