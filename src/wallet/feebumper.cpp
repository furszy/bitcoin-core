...
static feebumper::Result PreconditionChecks(const CWallet& wallet, const CWalletTx& wtx, bool require_mine, std::vector<bilingual_str>& errors) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    if (require_mine) {
        ...
        isminefilter filter = (wallet.GetLegacyScriptPubKeyMan() && wallet.IsWalletFlagSet(WALLET_FLAG_DISABLE_PRIVATE_KEYS))
                                || wallet.IsWalletFlagSet(WALLET_FLAG_EXTERNAL_SIGNER) ? ISMINE_WATCH_ONLY : ISMINE_SPENDABLE;
