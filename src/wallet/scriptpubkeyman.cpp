...
std::optional<MigrationData> LegacyScriptPubKeyMan::MigrateToDescriptor()
{
    ...
    WalletBatch batch(m_storage.GetDatabase());
    if (!batch.TxnBegin()) {
        WalletLogPrintf("Error during descriptors migration, cannot initialize db transaction\n");
        return std::nullopt;
    }
    ...
    if (!batch.TxnCommit()) {
        WalletLogPrintf("Error during descriptors migration, cannot commit db transaction\n");
        return std::nullopt;
    }
