...
util::Result<SelectionResult> AutomaticCoinSelection(const CWallet& wallet, CoinsResult& available_coins, const CAmount& value_to_select, const CCoinControl& coin_control, const CoinSelectionParams& coin_selection_params)
{
    ...
    util::Result<SelectionResult> res = [&] {
        ...
        // Only allow mixed output types on the first filter only.
        unsigned int index_for_allow_mixed_output_types = 1;
        // Fall back to using zero confirmation change (but with as few ancestors in the mempool as
        // possible) if we cannot fund the transaction otherwise.
        if (wallet.m_spend_zero_conf_change) {
            ordered_filters.emplace_back(CoinEligibilityFilter(0, 1, 2));
            ordered_filters.emplace_back(CoinEligibilityFilter(0, 1, std::min((size_t)4, max_ancestors/3), std::min((size_t)4, max_descendants/3)));
            ordered_filters.emplace_back(CoinEligibilityFilter(0, 1, max_ancestors/2, max_descendants/2));
            // If partial groups are allowed, relax the requirement of spending OutputGroups (groups
            // of UTXOs sent to the same address, which are obviously controlled by a single wallet)
            // in their entirety.
            ordered_filters.emplace_back(CoinEligibilityFilter(0, 1, max_ancestors-1, max_descendants-1, /*include_partial=*/true));
            // Try with unsafe inputs if they are allowed. This may spend unconfirmed outputs
            // received from other wallets.
            if (coin_control.m_include_unsafe_inputs) {
                ordered_filters.emplace_back(CoinEligibilityFilter(/*conf_mine=*/0, /*conf_theirs*/0, max_ancestors-1, max_descendants-1, /*include_partial=*/true));
            }
            // Try with unlimited ancestors/descendants. The transaction will still need to meet
            // mempool ancestor/descendant policy to be accepted to mempool and broadcasted, but
            // OutputGroups use heuristics that may overestimate ancestor/descendant counts.
            if (!fRejectLongChains) {
                ordered_filters.emplace_back(CoinEligibilityFilter(0, 1, std::numeric_limits<uint64_t>::max(),
        ...
        for (unsigned int i = 0; i < ordered_filters.size(); i++) {
            const auto& filter = ordered_filters[i];
