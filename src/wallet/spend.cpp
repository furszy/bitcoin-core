...
static CallResult<CreatedTransactionResult> CreateTransactionInternal(
        CWallet& wallet,
...
CallResult<CreatedTransactionResult> CreateTransaction(
        CWallet& wallet,
        const std::vector<CRecipient>& vecSend,
        int change_pos,
        const CCoinControl& coin_control,
        bool sign)
{
    ...
    TRACE4(coin_selection, normal_create_tx_internal, wallet.GetName().c_str(), res,
           res ? res.GetObjResult()->fee : 0, res ? res.GetObjResult()->change_pos : 0);
    if (!res) return res;
    const auto& txr_ungrouped = res.GetObjResult();
    // try with avoidpartialspends unless it's enabled already
    if (txr_ungrouped->fee > 0 /* 0 means non-functional fee rate estimation */ && wallet.m_max_aps_fee > -1 && !coin_control.m_avoid_partial_spends) {
        TRACE1(coin_selection, attempting_aps_create_tx, wallet.GetName().c_str());
        CCoinControl tmp_cc = coin_control;
        tmp_cc.m_avoid_partial_spends = true;
        if (auto res2 = CreateTransactionInternal(wallet, vecSend, change_pos, tmp_cc, sign)) {
            const auto& txr_grouped = res2.GetObjResult();
...
bool FundTransaction(CWallet& wallet, CMutableTransaction& tx, CAmount& nFeeRet, int& nChangePosInOut, bilingual_str& error, bool lockUnspents, const std::set<int>& setSubtractFeeFromOutputs, CCoinControl coinControl)
{
    ...
    const auto& txr = res.GetObjResult();
