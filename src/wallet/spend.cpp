        ...
        for (const auto& [type, outs] : coins.coins) {
            for (const COutput& output : outs) {
                ...
                for (const auto& sel_filter : filters) {
                    ...
                    filtered_groups[filter].push(group, type, OutputGroups::InsertGroupType::BOTH);
                }
            }
        }
        ...
        for (const auto& [script, groups] : groups_map) {
            ...
            for (auto group_it = groups.rbegin(); group_it != groups.rend(); group_it++) {
                ...
                for (const auto& sel_filter : filters) {
                    ...
                    filtered_groups[filter].push(group, type,
                                   positive_only ? OutputGroups::InsertGroupType::ONLY_POSITIVE : OutputGroups::InsertGroupType::MIXED_GROUPS);
                }
            }
        }
    };
...
util::Result<SelectionResult> AttemptSelection(const CAmount& nTargetValue, OutputGroups& groups,
