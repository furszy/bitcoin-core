...
{
    auto wallet = std::make_unique<CWallet>(context->m_node.chain.get(), "", CreateMockWalletDatabase());
    {
...
BOOST_FIXTURE_TEST_CASE(descriptors_wallet_detect_change_output, TestChain100Setup)
{
...

    {
        ...
        WalletDescriptor wdesc(std::move(desc), 0, 0, 0, 0, /*internal=*/true);
        ...
        coin_control.destChange = dest_change;
        auto op_tx2 = *Assert(CreateTransaction(*wallet, {{CScript() << OP_TRUE, 1 * COIN, true}}, -1, coin_control));
        BOOST_CHECK(!OutputIsChange(wallet, op_tx2.tx, op_tx2.change_pos));

        // Now add the descriptor and verify that change is detected
        WalletDescriptor wdesc2(std::move(desc2), 0, 0, 0, 0, /*internal=*/true);
...

    {
        ...
        for (const auto& it_wallet : {wallet.get(), external_wallet.get()}) it_wallet->blockConnected(block_info);
