...
bool CWallet::AddWalletFlags(uint64_t flags, WalletBatch& batch)
{
    LOCK(cs_wallet);
...
BResult<CTxDestination> CWallet::GetNewDestination(const OutputType type, const std::string label)
{
    ...
    WalletBatch batch(GetDatabase(), /*fFlushOnClose=*/true, /*initialize=*/false, /*is_txn=*/true);
...
std::shared_ptr<CWallet> CWallet::Create(WalletContext& context, const std::string& name, std::unique_ptr<WalletDatabase> database, uint64_t wallet_creation_flags, bilingual_str& error, std::vector<bilingual_str>& warnings)
{
    ...
    if (fFirstRun)
    {
        ...
        walletInstance->AddWalletFlags(wallet_creation_flags, batch);
