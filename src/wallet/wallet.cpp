...
bool CWallet::ApplyMigrationData(MigrationData& data, bilingual_str& error)
{
    ...
    auto func_store_addr = [](WalletBatch& batch, const CTxDestination& dest, const CAddressBookData& record){
        auto address{EncodeDestination(dest)};
        if (record.purpose) batch.WritePurpose(address, PurposeToString(*record.purpose));
        if (record.label) batch.WriteName(address, *record.label);
        for (const auto& [id, request] : record.receive_requests) {
            batch.WriteAddressReceiveRequest(dest, id, request);
        }
        if (record.previously_spent) batch.WriteAddressPreviouslySpent(dest, true);
    };
