...
bool CWallet::MigrateToSQLite(bilingual_str& error, bool& original_removed)
{
    ...
    // Create a temporary SQLite database next to the original one, so it can be renamed into
    // place within the same filesystem. Once all records are imported, it replaces the original one.
