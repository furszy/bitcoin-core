...
void CWallet::SetupDescriptorScriptPubKeyMans()
{
    ...
    if (!IsWalletFlagSet(WALLET_FLAG_EXTERNAL_SIGNER)) {
        if (!RunWithinTxn(GetDatabase(), /*process_desc=*/"setup descriptors", [&](WalletBatch& batch) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet){
            ...
            return true;
        })) throw std::runtime_error("Error: cannot execute db transaction for descriptors setup");
    } else {
        ExternalSigner signer = ExternalSignerScriptPubKeyMan::GetExternalSigner();
...
util::Result<void> CWallet::ApplyMigrationData(WalletBatch& local_wallet_batch, MigrationData& data)
{
    ...
    if (txids_to_delete.size() > 0) {
        auto res_removal{RemoveTxs(local_wallet_batch, txids_to_delete)};
        if (!res_removal) return util::Error{_("Error: Could not delete watchonly transactions. ") + util::ErrorString(res_removal)};
        ClearTxs(*res_removal); // Remove txs from memory
    }
