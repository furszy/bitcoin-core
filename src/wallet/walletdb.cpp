...
static DBErrors LoadLegacyWalletRecords(CWallet* pwallet, WalletBatch& wallet_batch, int last_client) EXCLUSIVE_LOCKS_REQUIRED(pwallet->cs_wallet)
{
    AssertLockHeld(pwallet->cs_wallet);
    DBErrors result = DBErrors::LOAD_OK;
    DatabaseBatch& batch = *wallet_batch.m_batch;
    ...
    if (!updated_metas.empty()) {
        ...
        for (const auto& pubkey : updated_metas) {
            if (!wallet_batch.WriteKeyMetadata(spkm->mapKeyMetadata.at(pubkey.GetID()), pubkey, true)) {
