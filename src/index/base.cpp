...
static bool FatalError(const char* fmt, const Args&... args)
{
...
std::any BaseIndex::ProcessBlock(const CBlockIndex* pindex, const Consensus::Params& consensus_params, const CBlock* block_data)
{
    ...
    if (!block_data) { // disk lookup if block data wasn't provided
        if (!ReadBlockFromDisk(block, pindex, consensus_params)) {
            return FatalError("%s: Failed to read block %s from disk",
    ...
    if (RequiresBlockUndoData()) {
        if (pindex->nHeight > 0 && !node::UndoReadFromDisk(block_undo, pindex)) {
            return FatalError("%s: Failed to read undo block data %s from disk",
                              __func__, pindex->GetBlockHash().ToString());
        }
...
std::vector<std::any> BaseIndex::ProcessBlocks(const CBlockIndex* start, const CBlockIndex* end, const Consensus::Params& consensus_params)
{
    std::vector<std::any> objs;
    do {
        objs.emplace_back(ProcessBlock(end, consensus_params));
        end = end->pprev;
...
void BaseIndex::ThreadSync()
{
    ...
    if (!m_synced) {
        ...
        bool parallel_sync_enabled = AllowParallelSyncEnabled() && m_thread_pool;
        ...
        while (true) {
            ...
            if (parallel_sync_enabled) {
                ...
                for (int i = 0; i < workers_count; i++) {
                    ...
                    // Async process
                    futures.emplace_back(m_thread_pool->Submit(std::bind(&BaseIndex::ProcessBlocks, this, it_start, it_end, consensus_params)));
                    // Update iterator
            ...
            // Otherwise, this is an active-wait, so we are processing blocks until all workers finish.
            ...
            std::packaged_task<std::vector<std::any>()> task(std::bind(&BaseIndex::ProcessBlocks, this, it_start, it_end, consensus_params));
...
void BaseIndex::BlockConnected(const std::shared_ptr<const CBlock>& block, const CBlockIndex* pindex)
{
    ...
    if (CustomPostProcessBlocks(ProcessBlock(pindex, Params().GetConsensus()))) {
