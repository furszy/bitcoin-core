...
static const CBlockIndex* NextSyncBlock(const CBlockIndex* pindex_prev, CChain& chain, int start_height) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (!pindex_prev) { // furszy: this is ugly..
...
void BaseIndex::Sync()
{
    ...
    if (pindex_next->nHeight == m_start_height && m_start_height > 0) {
        pindex = Assert(pindex_next->pprev);
    }
    ...
    std::shared_ptr<SyncContext> ctx = std::make_shared<SyncContext>();
    {
        ...
        for (int id = 0; id < num_tasks; ++id) {
            it_start = Assert(NextSyncBlock(it_start, m_chainstate->m_chain, m_start_height));
    ...
    while (!m_synced) {
        ...
        if (status == WorkerStatus::FINISHED) {
            ...
            pindex_next = NextSyncBlock(curr_tip, m_chainstate->m_chain, m_start_height);
