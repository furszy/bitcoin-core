...
static void http_request_cb(struct evhttp_request* req, void* arg)
{
    ...
    if (i != iend) {
        ...
        if (auto res = g_threadpool_http.Submit(std::move(item)); !res.has_value()) {
            const auto err_code = res.error();
            LogWarning("HTTP request rejected during server shutdown: '%s'", SubmitErrorString(err_code));
            Assume(err_code == ThreadPool::SubmitError::Interrupted);
