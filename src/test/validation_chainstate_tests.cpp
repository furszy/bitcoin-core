...
static void simulate_file_system_error(const fs::path& path, auto&& fn)
{
    const auto old_perms = fs::status(path).permissions();
    fs::permissions(path, fs::perms::none, fs::perm_options::replace);
    try {
        fn();
    } catch (...) {
        fs::permissions(path, old_perms, fs::perm_options::replace);
        throw;
    }
    fs::permissions(path, old_perms, fs::perm_options::replace);
}

static void check_ABC_triggers_fatal_error(const std::string& expected_log, node::NodeContext& node, Chainstate& chainstate)
{
...
BOOST_FIXTURE_TEST_CASE(activate_best_chain_connect_io_failure, TestChain100Setup)
{
    ...
    simulate_file_system_error(blk_file, [&]() {
        check_ABC_triggers_fatal_error("Failed to read block", m_node, chainstate);
    });

    // Inaccessible blocks directory must also trigger a fatal error.
    const auto parent_dir = blocks_dir.parent_path();
    simulate_file_system_error(parent_dir, [&]() {
        check_ABC_triggers_fatal_error("Failed to read block", m_node, chainstate);
    });
...
BOOST_FIXTURE_TEST_CASE(activate_best_chain_disconnect_io_failure, TestChain100Setup)
{
    ...
    const auto blk_file = blocks_dir / "blk00000.dat";
    simulate_file_system_error(blk_file, [&]() {
        check_ABC_triggers_fatal_error("Failed to disconnect block", m_node, chainstate);
    });

    // Inaccessible undo file during reorg must trigger fatal error.
    const auto rev_file = blocks_dir / "rev00000.dat";
    simulate_file_system_error(rev_file, [&]() {
        check_ABC_triggers_fatal_error("Failed to disconnect block", m_node, chainstate);
    });
