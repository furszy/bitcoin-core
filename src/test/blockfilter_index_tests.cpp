...
// A BlockFilterIndex subclass that simulates delays during 'CustomAppend'.
// This allows testing more complex scenarios, such as reorgs during initial sync.
class IndexBlockSim : public BlockFilterIndex
{
...
public:
    IndexBlockSim(std::unique_ptr<interfaces::Chain> chain, bool f_memory, int blocking_height) :
        BlockFilterIndex(std::move(chain), BlockFilterType::BASIC, /*n_cache_size=*/0, f_memory, /*f_wipe=*/false),
        m_blocking_height(blocking_height) {}
...
// Tests that a BlockFilterIndex can complete its initial sync even if a reorg occurs mid-sync.
...
BOOST_FIXTURE_TEST_CASE(initial_sync_reorg, BuildChainTestingSetup)
{
    ...
    IndexBlockSim index(interfaces::MakeChain(m_node), /*f_memory=*/true, blocking_height);
    index.SetProcessingBatchSize(1);
    BOOST_REQUIRE(index.Init());
    BOOST_REQUIRE(index.StartBackgroundSync());
    ...
    BOOST_CHECK(WaitUntil([&index] { return index.GetSummary().synced; }, /*timeout=*/5s));
...
BOOST_FIXTURE_TEST_CASE(shutdown_during_initial_sync, BuildChainTestingSetup)
{
    ...
    {
        // Create a filter index that will block during initial sync
        IndexBlockSim index(interfaces::MakeChain(m_node), /*f_memory=*/false, /*blocking_height=*/SHUTDOWN_HEIGHT);
        ...
        // Now verify locator is properly updated to the expected one
        WaitForHeight(&index, EXPECTED_LAST_SYNCED_BLOCK, /*timeout=*/200s);
    }
