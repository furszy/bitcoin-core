...
static void WalletMigration(benchmark::Bench& bench)
{
    ...
    for (int i = 0; i < NUM_WALLETS; ++i) {
        ...
        bilingual_str error;
        fs::path filename{fs::PathFromString(strprintf("legacy_%d", i))};
        auto database = MakeWalletDatabase(test_setup->m_path_root / filename, options, status, error);
    ...
    bench.epochs(NUM_WALLETS).run([&] {
        fs::path filename{fs::PathFromString(strprintf("legacy_%d", wallet_num))};
        util::Result<MigrationResult> res = MigrateLegacyToDescriptor(test_setup->m_path_root / filename, "", context);
        assert(res);
        assert(res->wallet);
