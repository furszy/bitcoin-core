...
static void WalletMigration(benchmark::Bench& bench)
{
    ...
    std::set<CScript> scripts_watch_only;
    for (int w = 0; w < NUM_WATCH_ONLY_ADDR; ++w) {
        CKey key;
        key.MakeNewKey(true);
        scripts_watch_only.insert(GetScriptForDestination(GetDestinationForKey(key.GetPubKey(), OutputType::LEGACY)));
    }
    bool res = WITH_LOCK(wallet->cs_wallet, return wallet->ImportScriptPubKeys(strprintf("watch"), scripts_watch_only,
                                           /*have_solving_data=*/false, /*apply_label=*/true, /*timestamp=*/1));
    assert(res);

    // Generate transactions and local addresses
    auto it_scripts = scripts_watch_only.begin();
    for (int j = 0; j < 400; ++j) {
        ...
        mtx.vout.emplace_back(COIN, *it_scripts);
        mtx.vin.resize(2);
        ...
        it_scripts++;
        if (it_scripts == scripts_watch_only.end()) it_scripts = scripts_watch_only.begin();
    }
