...
static std::vector<CAddress> ConvertSeeds(const std::vector<uint8_t> &vSeedsIn)
{
    ...
    while (!s.eof()) {
        CService endpoint;
        s >> endpoint;
        CAddress addr{endpoint, StatelessServicesFlags()};
...
void CConnman::ThreadDNSAddressSeed()
{
    ...
    for (const std::string& seed : seeds) {
        ...
        if (HaveNameProxy()) {
            ...
            ServiceFlags requiredServiceBits = StatelessServicesFlags();
        ...
        while (!interruptNet)
        {
            if (anchor && !m_anchors.empty()) {
                ...
                if (!addr.IsValid() || IsLocal(addr) || !IsReachable(addr) ||
                    !m_msgproc->HasDesirableServiceFlags(addr.nServices) ||
            ...
            if (!fFeeler && !m_msgproc->HasDesirableServiceFlags(addr.nServices)) {
                continue;
