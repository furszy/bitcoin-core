...
public:
    ...
    bool HasDesirableServiceFlags(ServiceFlags services) override;
    ...
    ServiceFlags GetDesirableServiceFlags(ServiceFlags services) override;
    ...
    if (!IsLimitedPeer(peer)) {
        m_block_tracker.for_pending(peer.m_id, [this, &state, count, &vBlocks](const uint256& block_hash) EXCLUSIVE_LOCKS_REQUIRED(::cs_main) {
            ...
            if (!index) {
                ...
                error("%s: pending block %s not found in block index", __func__, block_hash.ToString());
...
std::optional<std::string> PeerManagerImpl::FetchBlock(std::optional<NodeId> op_peer_id, const CBlockIndex& block_index)
{
    ...
    if (IsLimitedPeer(*peer)) return "Cannot fetch block from a limited peer";
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::VERSION) {
        ...
        if (pfrom.ExpectServicesFromConn() && !HasDesirableServiceFlags(nServices))
        {
        ...
        pfrom.m_has_all_wanted_services = HasDesirableServiceFlags(nServices);
    ...
    if (msg_type == NetMsgType::ADDR || msg_type == NetMsgType::ADDRV2) {
        ...
        for (CAddress& addr : vAddr)
        {
            ...
            if (!MayHaveUsefulAddressDB(addr.nServices) && !HasDesirableServiceFlags(addr.nServices))
                continue;
